...
# 线程池，每个池都按 LLM_CONCURRENCY 开，乘起来远超服务端上限，撞 429 后
# 走指数退避反而更慢。真正该约束的是「同时在飞的 HTTP 调用总数」，与调用点
# 嵌套几层无关，所以闸门放在这里而不是各个线程池上。
# 有了它，上层线程池可以放心开大：谁抢到令牌谁发车，没抢到的排队等。
#
# 上限实测（2026-08-20，scripts/probe_concurrency.py，生产同尺寸调用
# max_tokens=6000，每档 3 波稳态）：
#   并发   吞吐/s   延迟s   HTTP 429   失败
#     96   11.74     8.2          0      0   ← 最后一个干净档位
#    128   12.82    10.0        136     10
#    160   11.70    13.7        361     29
#    192   12.45    15.4        620     69
# 96 以上吞吐横盘在 12 次/秒左右，只多了限流与失败。故在飞总数上限取 96，
# rerun_both.sh 阶段一两进程并行、每进程 48。
#
# 此前注释写的「拐点 64」来自小调用（max_tokens=400）扫描，且档位按时间
# 顺序跑、与服务端负载混淆，不成立——真限流一定伴随 429，那轮全程为零。
